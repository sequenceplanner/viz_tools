...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let server_requests = node.create_service::<ManipulateDynamicMarker::Service>("manipulate_dynamic_marker")?;
    ...
    tokio::task::spawn(async move {
        let result = cube_handler_server(
            server_requests,
            markers_clone_2,
            sms_client,
            ...
            Some(request) => match request.message.command.as_str() {
                "update" => {
                    match manipulate_scene(&request.message, &sms_client).await {
                        true => {
                            match get_marker_data(&request.message.child_id, &marker_datas) {
...
fn get_marker_data(child_id: &str, marker_datas: &Arc<Mutex<Vec<MarkerData>>>) -> Option<MarkerData> {
    marker_datas.lock().unwrap().iter().find(|x| x.child_id == child_id).map(|x| x.clone())
}

fn add_marker(message: &r2r::viz_tools_msgs::srv::ManipulateDynamicMarker::Request, marker_datas: &Arc<Mutex<Vec<MarkerData>>>) -> bool {
...
fn update_marker(message: &r2r::viz_tools_msgs::srv::ManipulateDynamicMarker::Request, marker_datas: &Arc<Mutex<Vec<MarkerData>>>, new_marker: &MarkerData) -> bool {
    let old_marker_index = match get_marker_data(&message.child_id, marker_datas) {
        Some(_) => match marker_datas.lock() {
            Ok(y) => {
                match y.iter().position(|z| *z.child_id == message.child_id).map(|m| m as i32) {
                    Some(n) => n,
                    None => -1
                }
            },
...
fn remove_marker(message: &r2r::viz_tools_msgs::srv::ManipulateDynamicMarker::Request, marker_datas: &Arc<Mutex<Vec<MarkerData>>>) -> bool {
...
fn make_marker_data(message: &r2r::viz_tools_msgs::srv::ManipulateDynamicMarker::Request) -> MarkerData {
    MarkerData {
    ...
    loop {
                ...
                type_: match marker.use_primitive {
                    true => match marker.primitive_type {
                        ...
                        x => {
                            r2r::log_error!(
                                "dynamic_visualization",
                                "Invalid primitive marker type: {}", x
                            );
                            r2r::log_error!(
                                "dynamic_visualization",
        ...
        match publisher.publish(&array_msg) {
            ...
            Err(e) => {
                r2r::log_error!(
                    "dynamic_visualization",
                    "Publisher failed to send a message with: {}", e
                );
            }
        };
        timer.tick().await?; 
